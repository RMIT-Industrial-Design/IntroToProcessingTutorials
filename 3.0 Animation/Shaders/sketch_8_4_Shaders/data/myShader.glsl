...
float ringStep(float outerRadius, float innerRadius, vec2 coord, float rotationOffset) {
    ...
    float radialMultiplier = fract(atan(coord.y, coord.x) * 7.0 / 6.28 - 0.3 * (iGlobalTime * ring_speed) + rotationOffset);
...
void main()
{
    ...
    uv.x += 0.04 * cos(0.6 * uv.x + 0.7 * uv.y - 4.0*cos(0.3 * iGlobalTime) * (0.3 + 0.02*uv.y));
    ...
    float v = 1.0 - (ringStep(outer1, inner1, localUV, 0.0) + ringStep(fatness + 0.25 * cellValue2, 0.05 + 0.05 * cellValue2, localUV, iGlobalTime * 1.2));
